...
export async function GET(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    if (!attemptResult.success || !attemptResult.data || attemptResult.data.length === 0) {
      return NextResponse.json(
        {
          success: false,
    ...
    `;
    const questionsResult = await executeQuery(questionsQuery, { quizId: attempt.quiz_id });
    ...
    console.error(`Error processing GET /api/quizzes/attempts/${params.id}:`, error);
